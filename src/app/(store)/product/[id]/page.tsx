...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-bone flex items-center justify-center text-[10px] uppercase tracking-[0.6em] text-taupe animate-pulse">
      AETHER Loading Archive...
    </div>
  );
  
  if (!product) return (
    <div className="min-h-screen bg-bone flex items-center justify-center text-taupe font-serif italic">
  ...
  // Improved video detection for Supabase URLs
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24">
          ...
          <div className="space-y-6">
            <div className="relative aspect-[3/4] bg-white/[0.03] overflow-hidden border border-taupe/10 shadow-2xl">
              {isVideo(activeAsset) ? (
                <video 
                  key={activeAsset} // Key forces re-render when video changes
                  src={activeAsset} 
